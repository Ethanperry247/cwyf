...
""" Create a hazard profile from an input csv
        ...
        * headers in csv should be named: latitude,longitude,altitude,time (any order)
...
class HazardWeights():
    ...
    def calc_hazard(self):
        ...
        self.data["distance_between"] = rolling_apply(
            HazardWeights.distance_between, 2, self.data["latitude"], self.data["longitude"])
        ...
        self.data["turnRadius"] = rolling_apply(
            HazardWeights.turn_radius, 3, self.data["latitude"], self.data["longitude"])
    ...
    def to_json(self, filename="weights.json", minimal=True):
        if minimal:
            with open(filename, 'w') as outfile:
                json.dump(self.data[['latitude', 'longitude', 'hazard']].to_dict(
                    'records'), outfile)
        else:
            with open(filename, 'w') as outfile:
    ...
    def to_csv(self, filename="weights.csv", minimal=True):
        if minimal:
            self.data[['latitude', 'longitude', 'hazard']].to_csv(filename)
        else:
...
hazards = HazardWeights("my_route.csv") # my_route.csv is a csv w/ headers: latitude,longitude,altitude,time
